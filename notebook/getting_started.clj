...
;; If you want to change the default, see `config.edn.sample`. Rename it to
;; `config.edn` and set the values for a default LLM with the desired
;; parameters.

;; ### Prompt composition
;;
;; A use case showing some of the basic Bouquet functionality with
;; prompt templates.
;;
...
;; > Genre: Crime
;; > Synopsis:
;; > In the gritty underbelly of City of Shadows, corruption and crime reign
;; > supreme. The play follows the interconnected lives of several individuals,
;; > revealing the intricate web of deceit and darkness that engulfs the city.
...
;; To do this, let's define a template with a prompt and slots for data
;; insertion. *Bosquet* uses [Selmer](https://github.com/yogthos/selmer)
;; templating library for prompt definitions. *Selmer* provides rich templating
;; language, but all what is needed now is its `{{DATA}}` syntax to mark
;; spots in the text for data injection.
;;
;; Linked templates are defined in a map, where the map key is a variable name
;; that can be used to reference templates from each other.
